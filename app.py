...
def heartbeat():
    ...
    Clientul trimite periodic:
      ...
      - optional: clear_error=true pentru a șterge ultima eroare (manual sau după un run reușit)
    """
    data = request.get_json(force=True, silent=True) or {}
    email = (data.get("email") or "").strip().lower()
    fingerprint = (data.get("fingerprint") or "").strip()
    ...
    # NOTE:
    #  - nu suprascriem ultima eroare la fiecare heartbeat "idle" sau "run_started"
    #  - actualizăm last_error_* doar dacă:
    #       a) clear_error=True (ștergere)
    #       b) error_code este prezent și non-empty (setare/actualizare)
    raw_error_code = data.get("error_code", None)
    raw_error_msg = data.get("error_message", None)
    ...
    if clear_error:
        ...
        if raw_error_code is not None:
            c = str(raw_error_code).strip()
            if c:
                error_code = c[:64]
                error_msg = (str(raw_error_msg).strip()[:400] if raw_error_msg is not None else None)
                has_new_error = True
    ...
    # error tracking (streak + timestamp)
    try:
        prev_code = (row.get("last_error_code") or "").strip() or None
        prev_streak = int(row.get("error_streak") or 0)
    except Exception:
        prev_code = None
        prev_streak = 0
    ...
    if clear_error:
        ...
        if prev_code and prev_code == error_code:
            streak = min(prev_streak + 1, 9999)
        else:
            streak = 1
        payload.update(
            {
    ...
    try:
        _supabase_call(lambda: supabase.table("ops_client_state").update(payload).eq("email", email).execute())
    except Exception:
